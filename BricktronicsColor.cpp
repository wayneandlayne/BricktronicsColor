...
BricktronicsColor::BricktronicsColor(uint8_t clockPin, uint8_t dataPin):
    _clockPin(clockPin),
    _dataPin(dataPin),
    _pinMode(&::pinMode),
...
BricktronicsColor::BricktronicsColor(const BricktronicsSensorSettings &settings):
...
void BricktronicsColor::begin(void)
{
    begin(TYPE_COLORFULL);
}

void BricktronicsColor::begin(uint8_t modeType)
{
    _type = modeType;
...
uint8_t BricktronicsColor::getColor()
{
    _readSensor();
    _calibrate();
    return _calToColor();
...
void BricktronicsColor::printColor(uint8_t color)
{
...
void BricktronicsColor::_resetSensor()
{
    _setClock(1);
    _setData(1);
    delay(1);
...
void BricktronicsColor::_sendMode(uint8_t mode)
{
...
uint8_t BricktronicsColor::_readByte()
{
...
uint16_t BricktronicsColor::_calcCRC(uint16_t crc, uint16_t val)
{
    for (uint8_t i = 0; i < 8; i++)
    {
        if (((val ^ crc) & 1) != 0)
...
bool BricktronicsColor::_readCalibration()
{
...
bool BricktronicsColor::_checkSensor()
{
...
int BricktronicsColor::_readFullColorValue(uint8_t newClock)
{
...
void BricktronicsColor::_readSensor()
{
...
int BricktronicsColor::_readRawValue()
{
...
void BricktronicsColor::_calibrate()
{
    uint8_t calTab;
...
uint8_t BricktronicsColor::_calToColor()
{
